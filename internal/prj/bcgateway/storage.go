...
func (s *Storage) CreateOrder(order *prj.BcgatewayOrder) (id int64, err error) {
	order.UUID = utils.UUIDv4()

	query := `
INSERT INTO bcgateway_orders (uuid, acquirer_user_id, order_id, crypto_amount, compensate, status)
VALUES (?, ?, ?, ?, ?, ?)
RETURNING id;
`
